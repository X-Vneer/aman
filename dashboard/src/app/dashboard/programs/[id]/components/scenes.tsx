...
import { ActionIcon, Card, Group, Image, ScrollArea, Stack, Text } from "@mantine/core"
import { useTranslation } from "react-i18next"
import { Scene } from "../../@types"
import DeleteSceneButton from "./delete-scene"
import { CirclePlus, Clock, Film } from "lucide-react"
import { useParams } from "react-router-dom"

const Scenes = ({ scenes }: { scenes: Scene[] }) => {
  const { t } = useTranslation()
  const { id, locale } = useParams()
  return (
    <Stack gap={"xs"}>
      <Group justify="space-between">
        <Text className="!text-2xl">{t("programs.scenes.title")}</Text>
        <Group gap="xs">
          <Film size={16} />
          <Text c="dimmed">
            {scenes.length} {t("general.scene")}
          </Text>
        </Group>
      </Group>
      ...
      <ScrollArea>
        <Group py={"md"}>
          <ActionIcon
            component={Link}
            to={`/dashboard/programs/${id}/scenes/add`}
            ...
            <Stack align="center" gap={"xs"} component={"span"}>
              ...
              <span>{t("programs.scenes.add")}</span>
            </Stack>
          </ActionIcon>
            ...
            <Card shadow="sm" padding="md" radius="12" w={250}>
              <Stack>
                <Group justify="space-between" mb="xs">
                  <Text>
                    {index + 1}. {t("general.scene")}
                  </Text>
                  <Group gap={"4"}>
                    <DeleteSceneButton {...scene} />
                    <ActionIcon
                      color="secondary"
                      variant="subtle"
                      radius={"sm"}
                      component={Link}
                      to={`/dashboard/programs/${id}/scenes/${scene.id}`}>
                      <svg
                ...
                <div className="h-[125px] overflow-hidden rounded-lg">
                  <img src={scene.logo} alt="logo" className="h-full w-full object-cover" />
                </div>
                <Group gap="xs">
                  <Clock size={14} />
                ...
                {/* <Text ta={"center"}>{scene.title[locale as "ar" | "en"]}</Text> */}
              </Stack>
            </Card>
          ))}
        </Group>
