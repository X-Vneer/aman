...
import { ChevronDown, Info, MoreVertical, Trash2, User } from "lucide-react"
...
const TableCom = () => {
  ...
  newURLSearchParams.set("has_form", "true")

  const { data, status, error, refetch, isFetching } = useQuery({
    queryKey: ["users", newURLSearchParams.toString()],
    queryFn: async () => await GetUsers(newURLSearchParams),
  ...
  return (
    <>
            ...
            {status === "success"
              ? data.items.data.map((user, index) => (
                  <Table.Tr key={user.id + "" + index}>
                    ...
                    <Table.Td className="text-center">
                      <Menu width={170} shadow="lg" position="left-start">
                        <Menu.Target>
      ...
      {/* Modal moved outside the table to fix z-index issues */}
      <Modal
        withinPortal
        size={"lg"}
        opened={opened}
        onClose={close}
        centered
        title={t("user.table.personal-info")}
        zIndex={1000}>
        {selectedUser?.info && (
          <SimpleGrid cols={2}>
            <Stack gap={"xs"}>
              <Text c="#5A5A5A">{t("user.table.gender")}</Text>
              <Text className="text-lg">{selectedUser.info.gender}</Text>
            </Stack>
            <Stack gap={"xs"}>
              <Text c="#5A5A5A">{t("user.table.age")}</Text>
              <Text className="text-lg">{selectedUser.info.age}</Text>
            </Stack>
            <Stack gap={"xs"}>
              <Text c="#5A5A5A">{t("user.table.nationality")}</Text>
              <Text className="text-lg">{selectedUser.info.nationality}</Text>
            </Stack>
            <Stack gap={"xs"}>
              <Text c="#5A5A5A">{t("user.table.sector")}</Text>
              <Text className="text-lg">{selectedUser.info.sector}</Text>
            </Stack>
            <Stack gap={"xs"}>
              <Text c="#5A5A5A">{t("user.table.workplace")}</Text>
              <Text className="text-lg">{selectedUser.info.workplace}</Text>
            </Stack>
          </SimpleGrid>
        )}
      </Modal>
