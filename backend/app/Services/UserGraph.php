...
{ 
    ...
    private function findHourly() {
        ...
        foreach($this->langs as $lang){
            $items = User::select(DB::raw('HOUR(created_at) as hour'), DB::raw('count(id) as y'))
                ->where('lang', $lang)
    ...
    private function findDaily() {
        ...
        foreach($this->langs as $lang){
            $items = User::select(DB::raw('DAY(created_at) as day'), DB::raw('count(id) as y'))
            ->where('lang', $lang)
    ...
    private function findWeekly() { 
        ...
        foreach($this->langs as $lang){
            $items = User::select(DB::raw('WEEKDAY(created_at) as weekday'), DB::raw('count(id) as y'))
            ->where('lang', $lang)
    ...
    private function findMonthly() {
        ...
        foreach($this->langs as $lang){
            $items = User::select(DB::raw('MONTH(created_at) as month'), DB::raw('count(id) as y'))
                ->where('lang', $lang)
    ...
    private function findYearly() {
        ...
        foreach($this->langs as $lang){
            ...
            $items = User::select(DB::raw('YEAR(created_at) as year'), DB::raw('count(id) as y'))
                ->where('lang', $lang)
            ...
            foreach ($items as $item) {
                $this->yearly[Carbon::now()->year - $item->year + $years]["y_$lang"] = $item->y;
                $this->yearly[Carbon::now()->year - $item->year + $years]["y"] = $this->findTotal($this->yearly[Carbon::now()->year - $item->year + $years]);
            }
        } 
