...
use App\Http\Requests\ScenesStoreRequest;
use App\Http\Resources\ScenesEditResource;
...
class ScenesController extends BaseApiController {
    use IndexTrait, ShowTrait, EditTrait, DestroyTrait, ToggleActiveTrait;


    function __construct()
    {
        parent::__construct(Scenes::class);
    }
    ...
    public function store(ScenesStoreRequest $request)
    {
        // try {
    ...
    function edit($id) {
        $this->resource = ScenesEditResource::class;
        return $this->editInit($id, null, false);
    }

    public function update(ScenesStoreRequest $request, $id)
    {
        try {
            $inputs = $request->validated();
