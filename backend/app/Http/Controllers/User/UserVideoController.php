...
class UserVideoController extends BaseApiController {
    ...
    /**
     * Admin: reset all UserVideo records (with null certificate_number) for a given video.
     * Deletes their UserAnswers and resets answer progress.
     */
    public function resetByVideo(Request $request, $video_id)
    {
        if (! auth('admin')->check()) {
            return $this->sendResponse(false, null, trans('msg.notFound'), null, 403, $request);
        }

        $video = Video::withTrashed()->find($video_id);
        if (! $video) {
            return $this->sendResponse(false, null, trans('msg.notFound'), null, 404, $request);
        }
        ...
        try {
            DB::beginTransaction();
            $affected = $this->resetUserVideosForVideo($video);
            DB::commit();
        } catch (\Throwable $th) {
            DB::rollBack();
            CustomLogger::logInfo('resetUserVideosByVideo', 'Error', ['th' => $th->getMessage(), 'video_id' => $video_id]);
            return $this->sendResponse(false, null, trans('msg.technicalError'), null, 500, $request);
        }
    ...
    /**
     * Admin: reset every UserVideo record across all videos where certificate_number is null.
     * Deletes the related UserAnswers and resets answer progress per video.
     */
    public function resetAll(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            $videoIds = UserVideo::whereNull('certificate_number')
                ->pluck('video_id')
                ->unique()
                ->values();
            ...
            foreach ($videoIds as $videoId) {
                ...
                if (! $video) continue;
                $totalAffected += $this->resetUserVideosForVideo($video);
            }
            DB::commit();
        } catch (\Throwable $th) {
            DB::rollBack();
            CustomLogger::logInfo('resetAllUserVideos', 'Error', ['th' => $th->getMessage()]);
            return $this->sendResponse(false, null, trans('msg.technicalError'), null, 500, $request);
        }
    ...
    protected function resetUserVideosForVideo(Video $video): int
    {
        $userVideoIds = UserVideo::whereNull('certificate_number')
            ->where('video_id', $video->id)
            ->pluck('id')->toArray();

        if (empty($userVideoIds)) {
            return 0;
        }

        UserAnswer::whereIn('user_video_id', $userVideoIds)->delete();
        return UserVideo::whereIn('id', $userVideoIds)->update([
            'answer_average' => '00:00:00',
            'hearts' => 5,
            'total_questions' => $video->questions->count(),
            'correct_answers' => 0,
