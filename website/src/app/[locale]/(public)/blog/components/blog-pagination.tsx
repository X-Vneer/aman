...
import { blogListingHref, getBlogPaginationPages } from "../pagination-utils"
...
export async function BlogPagination({ currentPage, lastPage, from, to, total }: BlogPaginationProps) {
  const t = await getTranslations("blog.pagination")
  if (lastPage <= 1 || total === 0) return null

  const fromN = from ?? 0
  const toN = to ?? 0
  const pages = getBlogPaginationPages(currentPage, lastPage)
  const linkClass =
    ...
    <nav
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        {currentPage <= 1 ? (
          ...
          <Link href={blogListingHref(currentPage - 1)} className={navClass}>
            {t("previous")}
          </Link>
        )}
