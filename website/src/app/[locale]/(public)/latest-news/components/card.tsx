...
export function formatNewsPublishDate(publishDate: string, locale: string) {
...
export function NewsCard({ news, href, locale }: NewsCardProps) {
  ...
  return (
    <Link href={href} className="group block">
      <Card
        ...
        <img src={news.logo} alt={news.title} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative flex flex-col gap-4 bg-[#0a0a0a58] p-4 pt-6 text-white backdrop-blur-lg">
          ...
          <Card.Footer className="flex items-center justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <time className="text-xs font-semibold" dateTime={news.publish_date}>
                {dateLabel}
              </time>
            </div>
