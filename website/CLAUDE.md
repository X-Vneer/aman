...
- **Middleware is `src/proxy.ts`, not `middleware.ts`.** It runs `next-intl` `createMiddleware`, matcher restricted to `/` and `/(ar|en|fr|ur|fil|id)/:path*`. If you add a locale, update both `src/config/index.ts` (`LOCALES`) *and* the matcher regex in `src/proxy.ts`.
...
Six locales: `ar, en, fr, fil, id, ur` (`src/config/index.ts`). `ar` and `ur` are RTL. All app routes live under `src/app/[locale]/...` with `localePrefix: "always"` — every URL has a locale segment.
